...
static s32 serdes_config_init(struct i2c_client *client,struct cam *priv)
{
	...
	{	 
		...
		msleep(100);
		if((serdes_write_16b_reg(client, priv->ser1_addr, 0x0010, 0x21)) < 0)
		{
			dev_err (&client->dev, "%s(%d): Failed\n",
					__func__, __LINE__);
			return -EIO;
		...
		msleep(100);
		if((serdes_read_16b_reg(client, priv->ser1_addr, 0x0000, &slave_addr)) < 0)
		{
			dev_err (&client->dev, "%s(%d): Failed\n",
					__func__, __LINE__);
			return -EIO;
		...
		if(priv->ser1_addr != (slave_addr>>1)){
			/* Enabling Only LINKB */
	...
	{	
		...
		serdes_read_16b_reg(client, priv->ser2_addr, 0x0000, &slave_addr);
			debug_printk(" in the serdes read slave address is %u\n", slave_addr);
		if(slave_addr == priv->ser2_addr << 1)
		{
		...
		if((serdes_write_16b_reg(client, priv->ser1_addr, 0x0000, priv->ser2_addr<<1)) < 0)
		{
			dev_err (&client->dev, "%s(%d): Failed\n",
					__func__, __LINE__);
			return -EIO;
...
static uint8_t Mc_data_ctrl_ui[1024], Mc_ret_data_ctrl_ui[1024];
		...
		ISP_CTRL_INFO * mcu_ui_info, int index)
{
	...
	Mc_data_ctrl_ui[0] = CMD_SIGNATURE;
	Mc_data_ctrl_ui[1] = CMD_ID_GET_CTRL_UI_INFO;
	Mc_data_ctrl_ui[2] = payload_len >> 8;
	Mc_data_ctrl_ui[3] = payload_len & 0xFF;
	Mc_data_ctrl_ui[4] = errorcheck(&Mc_data_ctrl_ui[2], 2);

	cam_write(client, Mc_data_ctrl_ui, TX_LEN_PKT);

	Mc_data_ctrl_ui[0] = CMD_SIGNATURE;
	Mc_data_ctrl_ui[1] = CMD_ID_GET_CTRL_UI_INFO;
	Mc_data_ctrl_ui[2] = index >> 8;
	Mc_data_ctrl_ui[3] = index & 0xFF;
	Mc_data_ctrl_ui[4] = errorcheck(&Mc_data_ctrl_ui[2], 2);
	err = cam_write(client, Mc_data_ctrl_ui, 5);
	if (err != 0) {
		dev_err(&client->dev," %s(%d) Error - %d \n", __func__,
				__LINE__, err);
		ret = -EIO;
	...
	err = cam_read(client, Mc_ret_data_ctrl_ui, RX_LEN_PKT);
	if (err != 0) {
		dev_err(&client->dev," %s(%d) Error - %d \n", __func__,
				__LINE__, err);
		ret = -EIO;
	...
	/* Verify CRC */
	orig_crc = Mc_ret_data_ctrl_ui[4];
	calc_crc = errorcheck(&Mc_ret_data_ctrl_ui[2], 2);
	if (orig_crc != calc_crc) {
		dev_err(&client->dev," %s(%d) CRC 0x%02x != 0x%02x \n",
				__func__, __LINE__, orig_crc, calc_crc);
		ret = -EINVAL;
	...
	payload_len =
		((Mc_ret_data_ctrl_ui[2] << 8) | Mc_ret_data_ctrl_ui[3]) + HEADER_FOOTER_SIZE;
	errcode = Mc_ret_data_ctrl_ui[5];
	if (errcode != ERRCODE_SUCCESS) {
		dev_err(&client->dev," %s(%d) Errcode - 0x%02x \n",
				__func__, __LINE__, errcode);
		ret = -EINVAL;
	...
	memset(Mc_ret_data_ctrl_ui, 0x00, payload_len);
	err = cam_read(client, Mc_ret_data_ctrl_ui, payload_len);
	if (err != 0) {
		dev_err(&client->dev," %s(%d) Error - %d \n", __func__,
				__LINE__, err);
		ret = -EIO;
	...
	/* Verify CRC */
	orig_crc = Mc_ret_data_ctrl_ui[payload_len - 2];
	calc_crc =
		errorcheck(&Mc_ret_data_ctrl_ui[2], payload_len - HEADER_FOOTER_SIZE);
	if (orig_crc != calc_crc) {
		dev_err(&client->dev," %s(%d) CRC 0x%02x != 0x%02x \n",
				__func__, __LINE__, orig_crc, calc_crc);
		ret = -EINVAL;
	...
	/* Verify Errcode */
	errcode = Mc_ret_data_ctrl_ui[payload_len - 1];
	if (errcode != ERRCODE_SUCCESS) {
		dev_err(&client->dev," %s(%d) Errcode - 0x%02x \n",
				__func__, __LINE__, errcode);
		ret = -EIO;
	...
	strncpy((char *)mcu_ui_info->ctrl_ui_data.ctrl_ui_info.ctrl_name, &Mc_ret_data_ctrl_ui[2],MAX_CTRL_UI_STRING_LEN);

	mcu_ui_info->ctrl_ui_data.ctrl_ui_info.ctrl_ui_type = Mc_ret_data_ctrl_ui[34];
	mcu_ui_info->ctrl_ui_data.ctrl_ui_info.ctrl_ui_flags = Mc_ret_data_ctrl_ui[35] << 8 |
		Mc_ret_data_ctrl_ui[36];

	if (mcu_ui_info->ctrl_ui_data.ctrl_ui_info.ctrl_ui_type == V4L2_CTRL_TYPE_MENU) {
		mcu_ui_info->ctrl_ui_data.ctrl_menu_info.num_menu_elem = Mc_ret_data_ctrl_ui[37];
		...
		for (i = 0; i < mcu_ui_info->ctrl_ui_data.ctrl_menu_info.num_menu_elem; i++) {
			...
			strncpy((char *)mcu_ui_info->ctrl_ui_data.ctrl_menu_info.menu[i],
					&Mc_ret_data_ctrl_ui[38 +(i *MAX_CTRL_UI_STRING_LEN)], MAX_CTRL_UI_STRING_LEN);
...
static uint8_t Mc_data_list_ctrl[1024], Mc_ret_data_list_ctrls[1024];
		...
		ISP_CTRL_INFO * mcu_cam_ctrl, struct cam *priv)
{
	...
	while (retry-- > 0) {
		...
		Mc_data_list_ctrl[0] = CMD_SIGNATURE;
		Mc_data_list_ctrl[1] = CMD_ID_GET_CTRL_INFO;
		Mc_data_list_ctrl[2] = payload_len >> 8;
		Mc_data_list_ctrl[3] = payload_len & 0xFF;
		Mc_data_list_ctrl[4] = errorcheck(&Mc_data_list_ctrl[2], 2);

		err = cam_write(client, Mc_data_list_ctrl, TX_LEN_PKT);
		if (err != 0) {
			dev_err(&client->dev," %s(%d) Error - %d \n",
					__func__, __LINE__, err);
			continue;
		...
		Mc_data_list_ctrl[0] = CMD_SIGNATURE;
		Mc_data_list_ctrl[1] = CMD_ID_GET_CTRL_INFO;
		Mc_data_list_ctrl[2] = index >> 8;
		Mc_data_list_ctrl[3] = index & 0xFF;
		Mc_data_list_ctrl[4] = errorcheck(&Mc_data_list_ctrl[2], 2);
		err = cam_write(client, Mc_data_list_ctrl, 5);
		if (err != 0) {
			dev_err(&client->dev," %s(%d) Error - %d \n",
					__func__, __LINE__, err);
			continue;
		...
		err = cam_read(client, Mc_ret_data_list_ctrls, RX_LEN_PKT);
		if (err != 0) {
			dev_err(&client->dev," %s(%d) Error - %d \n",
					__func__, __LINE__, err);
			continue;
		...
		/* Verify CRC */
		orig_crc = Mc_ret_data_list_ctrls[4];
		calc_crc = errorcheck(&Mc_ret_data_list_ctrls[2], 2);
		if (orig_crc != calc_crc) {
			dev_err(&client->dev,
					" %s(%d) CRC 0x%02x != 0x%02x \n",
					__func__, __LINE__, orig_crc, calc_crc);
		...
		if (((Mc_ret_data_list_ctrls[2] << 8) | Mc_ret_data_list_ctrls[3]) == 0) {
			priv->num_ctrls = index;
			break;
		}

		payload_len =
			((Mc_ret_data_list_ctrls[2] << 8) | Mc_ret_data_list_ctrls[3]) +
			HEADER_FOOTER_SIZE;
		errcode = Mc_ret_data_list_ctrls[5];
		if (errcode != ERRCODE_SUCCESS) {
			dev_err(&client->dev,
					" %s(%d) Errcode - 0x%02x \n",
					__func__, __LINE__, errcode);
		...
		memset(Mc_ret_data_list_ctrls, 0x00, payload_len);
		err = cam_read(client, Mc_ret_data_list_ctrls, payload_len);
		if (err != 0) {
			dev_err(&client->dev," %s(%d) Error - %d \n",
					__func__, __LINE__, err);
			continue;
		...
		/* Verify CRC */
		orig_crc = Mc_ret_data_list_ctrls[payload_len - 2];
		calc_crc =
			errorcheck(&Mc_ret_data_list_ctrls[2],
					payload_len - HEADER_FOOTER_SIZE);
		if (orig_crc != calc_crc) {
			dev_err(&client->dev,
					" %s(%d) CRC 0x%02x != 0x%02x \n",
		...
		/* Verify Errcode */
		errcode = Mc_ret_data_list_ctrls[payload_len - 1];
		if (errcode != ERRCODE_SUCCESS) {
			dev_err(&client->dev,
					" %s(%d) Errcode - 0x%02x \n",
					__func__, __LINE__, errcode);
		...
		if(mcu_cam_ctrl != NULL) {
			...
			mcu_cam_ctrl[index].ctrl_id =
				Mc_ret_data_list_ctrls[2] << 24 | Mc_ret_data_list_ctrls[3] << 16 | Mc_ret_data_list_ctrls[4]
				<< 8 | Mc_ret_data_list_ctrls[5];
			mcu_cam_ctrl[index].ctrl_type = Mc_ret_data_list_ctrls[6];

			switch (mcu_cam_ctrl[index].ctrl_type) {
				case CTRL_STANDARD:
					mcu_cam_ctrl[index].ctrl_data.std.ctrl_min =
						Mc_ret_data_list_ctrls[7] << 24 | Mc_ret_data_list_ctrls[8] << 16
						| Mc_ret_data_list_ctrls[9] << 8 | Mc_ret_data_list_ctrls[10];

					mcu_cam_ctrl[index].ctrl_data.std.ctrl_max =
						Mc_ret_data_list_ctrls[11] << 24 | Mc_ret_data_list_ctrls[12] <<
						16 | Mc_ret_data_list_ctrls[13]
						<< 8 | Mc_ret_data_list_ctrls[14];

					mcu_cam_ctrl[index].ctrl_data.std.ctrl_def =
						Mc_ret_data_list_ctrls[15] << 24 | Mc_ret_data_list_ctrls[16] <<
						16 | Mc_ret_data_list_ctrls[17]
						<< 8 | Mc_ret_data_list_ctrls[18];

					mcu_cam_ctrl[index].ctrl_data.std.ctrl_step =
						Mc_ret_data_list_ctrls[19] << 24 | Mc_ret_data_list_ctrls[20] <<
						16 | Mc_ret_data_list_ctrls[21]
						<< 8 | Mc_ret_data_list_ctrls[22];
					break;
...
skip:
	err = of_property_read_u32(node, "deserializer-address", &deserAddr);
	if(err){
		dev_err(&client->dev,"desirializer addr value not found\n");
		return err;
	}
	...
	if(poc_enable > 0) {
		...
		msleep(500);		
		serdes_write_16b_reg(client, (uint8_t)deserAddr, 0x0010, 0x80);
		msleep(200);
	}
...
skip_trigger:
	...
	if(err){
		...
		/*Reading in serializer addresses*/

	err = of_property_read_u32(client->dev.of_node, "serializer-address_1", &readAddr1);
	if(err){
		dev_err(&client->dev,"serializer1 addr value not found\n");
		return err;
	}

	err = of_property_read_u32(client->dev.of_node, "serializer-address_2", &readAddr2);
	if(err){
		dev_err(&client->dev,"serializer2 addr value not found\n");
		return err;
	}
	

	
	priv->des_addr = (uint8_t)deserAddr;
	priv->ser1_addr = (uint8_t)readAddr1;
	priv->ser2_addr = (uint8_t)readAddr2;
...
{	
	...
	if(priv->phy == PHY_B){
		...
		if(serdes_write_16b_reg(client, priv->ser_addr, 0x0000, priv->ser1_addr<<1) < 0)
		{
			dev_err (&client->dev, "%s(%d): Failed\n",
					__func__, __LINE__);
			return -EIO;
