...
/** PWM is not required for e-CAM30_CUNANO " ***/
//#include "pca9685.h"  
		...
		if ((priv->mcu_cam_frmfmt[priv->frmfmt_mode].framerates[ret] ==
			...
			while (--retry >= 0) {
				err = mcu_stream_config(client, priv->format_fourcc, priv->frmfmt_mode,
							priv->frate_index);
				if (err < 0) {
				  	dev_err(&client->dev, "(%s) mcu_stream_config failure. retrying \n", __func__);
				  	continue;
				} else {
				  	break;
				}
			  ...
			  struct v4l2_subdev_format *format)
{
	 ...
	 * 30Hz frequency for MODE_2304x1296 or MODE_2304x1536 resolution
	 */
	if((mode == MODE_2304x1536 && priv->last_sync_mode == 2) ||
			(mode == MODE_2304x1296 && priv->last_sync_mode == 2 && 
...
static int ar0330_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if(ctrl->id == V4L2_CID_FRAME_SYNC) {
		if(ctrl->val == 1){ 
			//calibration_init(0);
			priv->last_sync_mode = 1;
		}else if(ctrl->val == 2 &&(  mode < MODE_2304x1296  ||
				( mode == MODE_2304x1296 &&  priv->mipi_lane_config == NUM_LANES_4 ))) {
			//calibration_init(1);
				...
				ISP_CTRL_INFO * mcu_ctrl)
{
	...
	if(mcu_ctrl->ctrl_id == V4L2_CID_EXPOSURE_AUTO)
		goto custom;
// frame sync is not required for e-cam30_cunano
...
static int mcu_mipi_configuration(struct i2c_client *client, struct ar0330 *priv, u8 cmd_id)
{
	...
	mc_data[0] = CMD_SIGNATURE;
        ...
        err = ar0330_write(client, mc_data, payload_len+3);
			...
			const struct i2c_device_id *id)
{
	...
	if (gpio < 0) {
		/* reset-gpio is not absoluctly needed */
		dev_info(&client->dev, "mipi select gpios not in DT\n");
		gpio = 0;
	} else {
	...
	if (err)
		return err;
/**** For e-CAM30_CUNANO calibration is not required ****/
#if 0
