...
skip_poc:	
	...
	err = of_property_read_u32(client->dev.of_node, "serializer-address_3", &readAddr3);
	if(err){
		dev_err(&client->dev, "serializer3 addr value not found\n");
		return err;
	}	
	...
	priv->ser3_addr = (uint8_t)readAddr3;
	...
	if(priv->phy == PHY_A || priv->phy == PHY_B) {
		...
		if(priv->phy == PHY_A) {
			...
			serdes_write_8b_reg(client, priv->ser1_addr, 0x04, 0x43);
			msleep(100);	
			serdes_write_16b_reg(client, priv->des_addr, 0x0B0D, 0x00);

			/* Change Serializer slave address */
			serdes_write_8b_reg(client, priv->ser1_addr, 0x00, priv->ser2_addr << 1);

			if(serdes_write_8b_reg(client, priv->ser2_addr, 0x04, 0x43) < 0) {
			...
			/* Change Serializer slave address */
			serdes_write_8b_reg(client, priv->ser1_addr, 0x00, priv->ser3_addr << 1);

			if(serdes_write_8b_reg(client, priv->ser3_addr, 0x04, 0x43) < 0) {
	...
	if(priv->phy == PHY_A) {
		...
		/* MCU RESET */
		if(serdes_write_8b_reg(client, priv->ser2_addr, 0x0D, 0x8F) < 0) {
				printk(" Error Accessing PHYA serializer \n");
				serdes_write_16b_reg(client, priv->des_addr, 0x0C04, 0x03);		
		...
		priv->ser_addr = priv->ser2_addr;
		serdes_write_8b_reg(client, priv->ser2_addr, 0x0F, MCU_ADDR2 << 1);
		serdes_write_8b_reg(client, priv->ser2_addr, 0x10, (MCU_ADDR1 << 1));				
		serdes_write_16b_reg(client, priv->des_addr, 0x0C04, 0x03);		
	} else if(priv->phy == PHY_B) {
		...
		priv->ser_addr = priv->ser3_addr;
		serdes_write_16b_reg(client, priv->des_addr, 0x0B04, 0x00);
		/* MCU RESET */
		if(serdes_write_8b_reg(client, priv->ser3_addr, 0x0D, 0x8F) < 0) {
				printk(" Error Accessing PHYA serializer \n");
				serdes_write_16b_reg(client, priv->des_addr, 0x0B04, 0x03);		
		...
		priv->ser_addr = priv->ser3_addr;
		serdes_write_8b_reg(client, priv->ser3_addr, 0x0F, MCU_ADDR3 << 1);
		serdes_write_8b_reg(client, priv->ser3_addr, 0x10, (MCU_ADDR1 << 1));				
		serdes_write_16b_reg(client, priv->des_addr, 0x0B04, 0x03);		
	}			
	...
	if(priv->phy == PHY_A) {
		serdes_write_8b_reg(client, priv->ser2_addr, 0x04, 0x83);
	} else if(priv->phy == PHY_B) {
		serdes_write_8b_reg(client, priv->ser3_addr, 0x04, 0x83);
	}						
	msleep(100);		
