...
#define IMX335_MAX_BLACK_LEVEL_12BIT 0xFFC
#define IMX335_MAX_BLACK_LEVEL_10BIT 0x3FF
...
static int imx335_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx335_custom_ctrl_ops = {
	.s_ctrl = imx335_set_custom_ctrls,
};
...
static struct v4l2_ctrl_config imx335_custom_ctrl_list[] = {
    {
        .ops = &imx335_custom_ctrl_ops,
        .id = TEGRA_CAMERA_CID_DATA_RATE,
        .name = "Data Rate",
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx335_data_rate_menu) - 1,
        .def = 0,
        .qmenu = imx335_data_rate_menu,
    },
    {
        .ops = &imx335_custom_ctrl_ops,
        ...
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx335_test_pattern_menu) - 1,
        .def = 0,
        .qmenu = imx335_test_pattern_menu,
    },
};
                                ...
                                u16 addr_low, u8 number_of_registers, u64 val)
{
    ...
    for (i = 0; i < number_of_registers; i++) {
        err = imx335_write_reg(s_data, addr_low + i, (u8)(val >> (i * 8)));
        if (err) {
...
static int imx335_write_table(struct imx335 *priv,
				const imx335_reg table[])
{
...
static int imx335_update_ctrl(struct tegracam_device *tc_dev, u64 val)
{
...
static int imx335_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	...
	switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			...
			break;
		default:
			if (val > IMX335_MAX_BLACK_LEVEL_10BIT)
				val = IMX335_MAX_BLACK_LEVEL_10BIT;
			err = imx335_update_ctrl(tc_dev, val);
			black_level_reg = val;
	}

    err = imx335_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, black_level_reg);
	if (err){
...
static int imx335_verify_data_rate(struct tegracam_device *tc_dev)
{
    ...
    if (s_data->numlanes == IMX335_TWO_LANE_MODE 
            && priv->data_rate == IMX335_891_MBPS 
...
modify_ctrl:
    ...
    priv->data_rate = IMX335_1188_MBPS;
 ...
 * Set streaming mode 
 */
static int imx335_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx335 *priv = (struct imx335 *)tc_dev->priv;
...
static int imx335_set_sync_feature(struct tegracam_device *tc_dev, u32 val)
{
	struct imx335 *priv = (struct imx335 *)tc_dev->priv;
...
static struct tegracam_ctrl_ops imx335_ctrl_ops = {
    ...
    .set_streaming_mode = imx335_set_streaming_mode,
    .set_operation_mode = imx335_set_operation_mode,
    .set_sync_feature = imx335_set_sync_feature,
...
static int imx335_power_on(struct camera_common_data *s_data)
{
    ...
    /* Power ON sequence according to IMX335 datasheet */

	if (pw->dvdd) {
		err = regulator_enable(pw->dvdd);
		if (err)
			goto imx335_dvdd_fail;
	}

	if (pw->iovdd) {
		err = regulator_enable(pw->iovdd);
		if (err)
			goto imx335_iovdd_fail;
	}

	if (pw->avdd) {
		err = regulator_enable(pw->avdd);
		if (err)
			goto imx335_avdd_fail;
	}
...
static int imx335_power_off(struct camera_common_data *s_data)
{
     ...
     * in the chapter "XVS/XHS IO setting" in the IMX335 Synchronizing 
     * Sensors App Note
     */         
    ...
    /* Power OFF sequence according to IMX335 datasheet */
...
static int imx335_communication_verify(struct tegracam_device *tc_dev)
{
	...
	err = imx335_read_buffered_reg(s_data, BLKLEVEL_LOW, 2, &black_level);
    if (err) {
...
/**
 * IMX335 allows for multiple data rates. Available data rates are described
 * in the chapter "Readout Drive mode" in the IMX335 datasheet
 */
static int imx335_set_data_rate(struct tegracam_device *tc_dev)
{
	struct imx335 *priv = (struct imx335 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
    int err;
    ...
    err = imx335_write_table(priv, data_rate_table[priv->data_rate]);
    if (err) 
        goto fail;
 ...
 */
static int imx335_set_sync_mode(struct tegracam_device *tc_dev)
{
	struct imx335 *priv = (struct imx335 *)tc_dev->priv;
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
...
static void imx335_enable_second_slave_address(struct imx335 *ack_private)
{
    ...
    err = imx335_write_reg(ack_private->s_data, 
                                SECOND_SLAVE_ADD, 3);
    if (err)
 ...
 */
static bool imx335_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx335 *priv = (struct imx335 *)tc_dev->priv;
    struct imx335 *current_priv;
	struct device *dev = tc_dev->dev;
    u8 streaming_mode = priv->current_streaming_mode;

    list_for_each_entry(current_priv, &imx335_sensor_list, entry) {
 ...
 */
static bool imx335_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx335 *priv = (struct imx335 *)tc_dev->priv;
    struct imx335 *current_priv; 
	struct device *dev = tc_dev->dev;
 ...
 */
int imx335_check_unsupported_mode(struct camera_common_data *s_data,
...
static int imx335_set_mode(struct tegracam_device *tc_dev)
{
    ...
    err = imx335_set_operation_mode(tc_dev, priv->current_operation_mode);
    if (err) {
        dev_err(dev, "%s: unable to operation mode\n", __func__);
        return err;   
    }

    err = imx335_set_sync_mode(tc_dev);
    if (err) {
    ...
    err = imx335_set_data_rate(tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to set data rate\n", __func__);
        return err;
    }
    ...
    if (!imx335_is_stream_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
        return -EPERM;
    }

    if (!imx335_is_pin_configuration_valid(tc_dev)) {
...
static int imx335_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx335_write_table(priv,
		mode_table[IMX335_MODE_START_STREAM]);
	if (err)
		return err;
...
static int imx335_ctrls_init(struct tegracam_device *tc_dev)
{
	struct imx335 *priv = (struct imx335 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = priv->s_data;
    ...
    numctrls = ARRAY_SIZE(imx335_custom_ctrl_list);

    for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx335_custom_ctrl_list[i];
			...
			const struct i2c_device_id *id)
{
    ...
    priv->s_data->blklvl_max_range = IMX335_MAX_BLACK_LEVEL_12BIT;
    ...
    err = imx335_ctrls_init(tc_dev);
	if (err) {
		dev_err(dev, "tegra camera custom ctrl init failed\n");
		return err;
	}

    err = imx335_update_ctrl(tc_dev, priv->black_level);
    if (err)
		return err;
