...
#define IMX464_MAX_BLACK_LEVEL 0x3FF
...
static int imx464_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx464_custom_ctrl_ops = {
	.s_ctrl = imx464_set_custom_ctrls,
};
...
static struct v4l2_ctrl_config imx464_custom_ctrl_list[] = {
    {
        .ops = &imx464_custom_ctrl_ops,
        .id = TEGRA_CAMERA_CID_DATA_RATE,
        .name = "Data Rate",
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx464_data_rate_menu) - 1,
        .def = 0,
        .qmenu = imx464_data_rate_menu,
    },
    {
        .ops = &imx464_custom_ctrl_ops,
        ...
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx464_test_pattern_menu) - 1,
        .def = 0,
        .qmenu = imx464_test_pattern_menu,
    },
};
...
static inline int imx464_read_reg(struct camera_common_data *s_data,
				u16 addr, u8 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
static int imx464_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
                                ...
                                u16 addr_low, u8 number_of_registers, u64 val)
{
    ...
    for (i = 0; i < number_of_registers; i++) {
        err = imx464_write_reg(s_data, addr_low + i, (u8)(val >> (i * 8)));
        if (err) {
...
static int imx464_write_table(struct imx464 *priv,
				const imx464_reg table[])
{
...
static int imx464_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
    ...
    err = imx464_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, val);
	if (err){
...
static int imx464_set_gain(struct tegracam_device *tc_dev, s64 val)
{
    ...
    if (priv->broadcast_ctrl == BROADCAST)      
        err = imx464_broadcast_buffered_reg(s_data, 
                                             GAIN_LOW, 2, gain);
    else {
        err = imx464_write_buffered_reg(s_data, 
                                            GAIN_LOW, 2, gain);
    }
	if (err){
...
static int imx464_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
    ...
    integration_time_line = DIV_ROUND_CLOSEST(val
                                     * IMX464_K_FACTOR, priv->line_time);

	reg_shr0 = priv->frame_length - integration_time_line;

    if (reg_shr0 < IMX464_MIN_SHR0_LENGTH)
        reg_shr0 = IMX464_MIN_SHR0_LENGTH;
	else if (reg_shr0 > (priv->frame_length - IMX464_MIN_INTEGRATION_LINES))
		reg_shr0 = priv->frame_length - IMX464_MIN_INTEGRATION_LINES;

    if (priv->broadcast_ctrl == BROADCAST)        
        err = imx464_broadcast_buffered_reg(s_data, SHR0_LOW, 3, reg_shr0);
    else
        err = imx464_write_buffered_reg (s_data, SHR0_LOW, 3, reg_shr0);
    if (err) {
...
static int imx464_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
    ...
    frame_length = (((u64)mode->control_properties.framerate_factor *
                              IMX464_G_FACTOR) / (val * priv->line_time));
    ...
    exposure_min_range = IMX464_MIN_INTEGRATION_LINES
                                            * priv->line_time / IMX464_K_FACTOR; 

    exposure_max_range = (priv->frame_length - IMX464_MIN_SHR0_LENGTH)
                                            * priv->line_time / IMX464_K_FACTOR;
...
static int imx464_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
    ...
    if (val) {
        err = imx464_write_table(priv, mode_table[IMX464_EN_PATTERN_GEN]);
        if (err) 
            goto fail;
        ...
        err = imx464_write_table(priv, mode_table[IMX464_DIS_PATTERN_GEN]);
        if (err) 
            goto fail;
    }
 ...
 * Set streaming mode 
 */
static int imx464_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx464 *priv = (struct imx464 *)tc_dev->priv;
...
static int imx464_set_sync_feature(struct tegracam_device *tc_dev, u32 val)
{
	struct imx464 *priv = (struct imx464 *)tc_dev->priv;
...
static struct tegracam_ctrl_ops imx464_ctrl_ops = {
    ...
    .set_streaming_mode = imx464_set_streaming_mode,
    .set_operation_mode = imx464_set_operation_mode,
    .set_sync_feature = imx464_set_sync_feature,
...
static int imx464_power_on(struct camera_common_data *s_data)
{
    ...
    /* Power ON sequence according to IMX464 datasheet */

	if (pw->dvdd) {
		err = regulator_enable(pw->dvdd);
		if (err)
			goto imx464_dvdd_fail;
	}

	if (pw->iovdd) {
		err = regulator_enable(pw->iovdd);
		if (err)
			goto imx464_iovdd_fail;
	}

	if (pw->avdd) {
		err = regulator_enable(pw->avdd);
		if (err)
			goto imx464_avdd_fail;
	}
...
static int imx464_power_off(struct camera_common_data *s_data)
{
     ...
     * in the chapter "XVS/XHS IO setting" in the IMX464 Synchronizing 
     * Sensors App Note
     */         
    ...
    /* Power OFF sequence according to IMX464 datasheet */
...
static struct camera_common_pdata *imx464_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
/**
 * IMX464 allows for multiple data rates. Available data rates are described
 * in the chapter "Readout Drive mode" in the IMX464 datasheet
 */
static int imx464_set_data_rate(struct tegracam_device *tc_dev)
{
	struct imx464 *priv = (struct imx464 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
    int err;
    ...
    err = imx464_write_table(priv, data_rate_table[priv->data_rate]);
    if (err) 
        goto fail;
 ...
 */
static int imx464_set_sync_mode(struct tegracam_device *tc_dev)
{
	struct imx464 *priv = (struct imx464 *)tc_dev->priv;
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
...
static void imx464_enable_second_slave_address(struct imx464 *ack_private)
{
    ...
    err = imx464_write_reg(ack_private->s_data, 
                                SECOND_SLAVE_ADD, 7);
    if (err)
 ...
 */
static bool imx464_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx464 *priv = (struct imx464 *)tc_dev->priv;
    struct imx464 *current_priv;
	struct device *dev = tc_dev->dev;
    u8 streaming_mode = priv->current_streaming_mode;

    list_for_each_entry(current_priv, &imx464_sensor_list, entry) {
 ...
 */
static bool imx464_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx464 *priv = (struct imx464 *)tc_dev->priv;
    struct imx464 *current_priv; 
	struct device *dev = tc_dev->dev;
...
static int imx464_update_init_ctrl(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
    struct v4l2_ctrl *ctrl;
	struct device *dev = tc_dev->dev;
    ...
    err = imx464_read_buffered_reg(s_data, BLKLEVEL_LOW, 2, &val);
    if (err) {
...
static int imx464_set_mode(struct tegracam_device *tc_dev)
{
    ...
    err = imx464_set_operation_mode(tc_dev, priv->current_operation_mode);
    if (err) {
        dev_err(dev, "%s: unable to operation mode\n", __func__);
        return err;   
    }

    err = imx464_set_sync_mode(tc_dev);
    if (err) {
    ...
    err = imx464_set_data_rate(tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to set data rate\n", __func__);
        return err;
    }
    ...
    err = imx464_update_init_ctrl(tc_dev);
    if (err)
		return err;

    if (!imx464_is_stream_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
        return -EPERM;
    }

    if (!imx464_is_pin_configuration_valid(tc_dev)) {
...
static int imx464_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx464_write_table(priv,
		mode_table[IMX464_MODE_START_STREAM]);
	if (err)
		return err;
...
static int imx464_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	usleep_range(priv->frame_length * priv->line_time / IMX464_K_FACTOR, 
                priv->frame_length * priv->line_time / IMX464_K_FACTOR + 1000);
...
static int imx464_board_setup(struct imx464 *priv)
{
    ...
    priv->min_frame_length = IMX464_DEFAULT_HEIGHT
                                + IMX464_MIN_FRAME_LENGTH_DELTA;
...
static int imx464_ctrls_init(struct tegracam_device *tc_dev)
{
	struct imx464 *priv = (struct imx464 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = priv->s_data;
    ...
    numctrls = ARRAY_SIZE(imx464_custom_ctrl_list);

    for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx464_custom_ctrl_list[i];
			...
			const struct i2c_device_id *id)
{
	...
	priv = devm_kzalloc(dev,
			sizeof(struct imx464), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
    ...
    priv->s_data->blklvl_max_range = IMX464_MAX_BLACK_LEVEL;
...
static int
imx464_remove(struct i2c_client *client)
{
...
static const struct i2c_device_id imx464_id[] = {
	{ "imx464", 0 },
	{ }
};
