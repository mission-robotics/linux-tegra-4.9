...
#define IMX290_K_FACTOR 1000LL
#define IMX290_M_FACTOR 1000000LL
#define IMX290_G_FACTOR 1000000000LL
#define IMX290_T_FACTOR 1000000000000LL

#define IMX290_MAX_GAIN_DEC 240
#define IMX290_MAX_GAIN_DB  72

#define IMX290_MAX_BLACK_LEVEL 0x1FF

#define IMX290_MIN_SHR0_LENGTH 1
#define IMX290_MIN_INTEGRATION_LINES 1

#define IMX290_MAX_CSI_LANES 4
#define IMX290_TWO_LANE_MODE 2

#define IMX290_INCK 74250000LL

/**
 * list HEAD of private data of all probed sensors on the platform 
 */
LIST_HEAD(imx290_sensor_list);
...
static int imx290_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx290_custom_ctrl_ops = {
	.s_ctrl = imx290_set_custom_ctrls,
};

const char * const imx290_data_rate_menu[] = {
    [IMX290_HIGH_DATA_RATE] = "891/594 Mbps/lane",
    [IMX290_MID_DATA_RATE]  = "445.5/297 Mbps/lane",
    [IMX290_LOW_DATA_RATE]  = "222.75/148.5 Mbps/lane",
};
...
static const char * const imx290_test_pattern_menu[] = {
    ...
    [5]   = "Gradiation Pattern 1",
    [6]   = "Gradiation Pattern 2",
    [7]   = "000/555h Toggle Pattern",
};

static struct v4l2_ctrl_config imx290_custom_ctrl_list[] = {
    {
        .ops = &imx290_custom_ctrl_ops,
        .id = TEGRA_CAMERA_CID_DATA_RATE,
        .name = "Data Rate",
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx290_data_rate_menu) - 1,
        .def = 0,
        .qmenu = imx290_data_rate_menu,
    },
    {
        .ops = &imx290_custom_ctrl_ops,
 ...
 */
static int imx290_read_buffered_reg(struct camera_common_data *s_data,
                                 u16 addr_low, u8 number_of_registers, u64 *val)
{
    ...
    *val = 0;
    ...
    if (!s_data->group_hold_active){
        err = imx290_write_reg(s_data, REGHOLD, 0x01);
        if (err) {
    ...
    for (i = 0; i < number_of_registers; i++) {
        err = imx290_read_reg(s_data, addr_low + i, &reg);
        *val += reg << (i * 8);
        if (err) {
    ...
    if (!s_data->group_hold_active){
        err = imx290_write_reg(s_data, REGHOLD, 0x00);
        if (err) {
 ...
 */
static int imx290_write_buffered_reg(struct camera_common_data *s_data, 
                                u16 addr_low, u8 number_of_registers, u64 val)
{
    int err, i;
	struct device *dev = s_data->dev;
    ...
    for (i = 0; i < number_of_registers; i++) {
        err = imx290_write_reg(s_data, addr_low + i, (u8)(val >> (i * 8)));
        if (err) {
...
static int imx290_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
    ...
    err = imx290_write_reg(s_data, REGHOLD, val);
    if (err) {
        dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
        return err;
    }
...
static int imx290_update_ctrl(struct tegracam_device *tc_dev, u64 val)
{
...
static int imx290_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

    err = imx290_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, val);
	if (err){
...
static int imx290_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	/* translate value */
	gain = val * IMX290_MAX_GAIN_DEC /
                 (IMX290_MAX_GAIN_DB *
                     mode->control_properties.gain_factor);

    
    err = imx290_write_reg(s_data, GAIN, gain);
...
static int imx290_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	integration_time_line = (val * IMX290_K_FACTOR) / priv->line_time ;

	reg_shs1 = priv->frame_length - integration_time_line;

    if (reg_shs1 < IMX290_MIN_SHR0_LENGTH)
        reg_shs1 = IMX290_MIN_SHR0_LENGTH;
	else if (reg_shs1 > (priv->frame_length - IMX290_MIN_INTEGRATION_LINES - 1))
		reg_shs1 = priv->frame_length - IMX290_MIN_INTEGRATION_LINES - 1;

    err = imx290_write_buffered_reg (s_data, SHS1_LOW, 3, reg_shs1);
	...
	dev_dbg(dev,
     ...
     __func__, val, integration_time_line, reg_shs1, priv->frame_length);
...
static int imx290_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
    ...
    frame_length = (((u64)mode->control_properties.framerate_factor * 
                              IMX290_G_FACTOR) / (val * priv->line_time));
    ...
    exposure_min_range = IMX290_MIN_INTEGRATION_LINES 
                                            * priv->line_time / IMX290_K_FACTOR; 

    exposure_max_range = (priv->frame_length - IMX290_MIN_SHR0_LENGTH) 
                                            * priv->line_time / IMX290_K_FACTOR;
    fm_update_ctrl_range(tc_dev, TEGRA_CAMERA_CID_EXPOSURE,
                             exposure_min_range, exposure_max_range);

    err = imx290_write_buffered_reg(s_data, VMAX_LOW, 3, priv->frame_length);
    if (err) {
        dev_err(dev, "%s: failed to set frame length\n", __func__);
        return err;
    }
 ...
 * in the IMX290 support package
 */
static int imx290_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev; 
    ...
    if (val) {
        err = imx290_write_table(priv, mode_table[IMX290_EN_PATTERN_GEN]);
        if (err) 
            goto fail;
        err = imx290_write_reg(s_data, PGMODE_COLORWIDTH_PGTHRU_PGREGEN, 
                                (u8)((val << 4) | 0x01));
        if (err) 
            goto fail;  
    } else {
        err = imx290_write_table(priv, mode_table[IMX290_DIS_PATTERN_GEN]);
        if (err) 
            goto fail;
    }
...
/**
 * Update max framerate range is needed if data rate is modified 
 */
static int imx290_update_framerate_range(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
    ...
    switch (s_data->mode) {
        case IMX290_MODE_1920x1080:
            delta = IMX290_MIN_FRAME_DELTA_1080p;
        break;
        case IMX290_MODE_1280x720:
            delta = IMX290_MIN_FRAME_DELTA_720p;
        break;
        case IMX290_MODE_crop_640x480:
            delta = IMX290_MIN_FRAME_DELTA_CROP;
        break;
        default:
            delta = IMX290_MIN_FRAME_DELTA_1080p;
    }

    priv->min_frame_length = s_data->fmt_height + delta;

    max_framerate = (IMX290_G_FACTOR * IMX290_M_FACTOR) /
 ...
 * in the IMX290 datasheet
 */
static int imx290_verify_data_rate(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
 ...
 * Set streaming mode 
 */
static int imx290_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx290 *priv = (struct imx290 *)tc_dev->priv;
 ...
 */
static int imx290_set_operation_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx290 *priv = (struct imx290 *)tc_dev->priv;
...
static struct tegracam_ctrl_ops imx290_ctrl_ops = {
	...
	.set_frame_rate = imx290_set_frame_rate,
	.set_group_hold = imx290_set_group_hold,
    .set_test_pattern = imx290_set_test_pattern,
    .set_streaming_mode = imx290_set_streaming_mode,
    .set_operation_mode = imx290_set_operation_mode,
    .set_black_level = imx290_set_black_level,
};
...
static int imx290_power_on(struct camera_common_data *s_data)
{
    ...
    /* Power ON sequence according to IMX290 datasheet */
...
static int imx290_power_off(struct camera_common_data *s_data)
{
     ...
     * in the chapter "XVS/XHS IO setting" in the IMX290 Synchronizing 
     * Sensors App Note
     */         
    err = imx290_write_reg(s_data, XHSOUTSEL_XVSOUTSEL, 0x0);
    if (err) {
    ...
    /* Power OFF sequence according to IMX290 datasheet */
 ...
 */
static int imx290_communication_verify(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
    ...
    err = imx290_read_buffered_reg(s_data, VMAX_LOW, 2, &vmax);
    if (err) {
	...
	err = imx290_read_buffered_reg(s_data, BLKLEVEL_LOW, 2, &black_level);
    if (err) {
...
static struct camera_common_pdata *imx290_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
static int imx290_set_pixel_format(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
    struct camera_common_data *s_data = tc_dev->s_data;
	...
	case MEDIA_BUS_FMT_SRGGB10_1X10:
        err = imx290_write_table(priv, mode_table[IMX290_10BIT_MODE]);
		break;
	case MEDIA_BUS_FMT_SRGGB12_1X12:
        err = imx290_write_table(priv, mode_table[IMX290_12BIT_MODE]);
		break;
	default:
 ...
 */
static int imx290_set_csi_lane_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
    int err;
    ...
    if (s_data->numlanes == IMX290_TWO_LANE_MODE) {
        err = imx290_write_reg(s_data, PHYSICAL_LANE_NUM, 1);
        if (err) {
            dev_err(dev, "%s: error setting two lane mode\n", __func__);
            return err;
        }
        
        err = imx290_write_reg(s_data, CSI_LANE_MODE, 1);
        if (err) {
            dev_err(dev, "%s: error setting two lane mode\n", __func__);
            return err;
        }
 ...
 * Calculate 1H time
 */
static int imx290_calculate_line_time(struct tegracam_device *tc_dev)
{
	struct imx290 *priv = (struct imx290 *)tc_dev->priv;
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
    ...
    err = imx290_read_buffered_reg(s_data, HMAX_LOW, 2, &hmax);
    if (err) {
        dev_err(dev, "%s: unable to read hmax\n", __func__);
        return err;
    }
    
    priv->line_time = ((hmax*IMX290_G_FACTOR) / (IMX290_INCK))/2;
...
static int imx290_set_data_rate_720p(struct tegracam_device *tc_dev)
{
    struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
    struct device *dev = tc_dev->dev;
    struct camera_common_data *s_data = tc_dev->s_data;
    int err;
    u64 hmax;
    u8 csi_lane_koef = IMX290_MAX_CSI_LANES / s_data->numlanes;
    
    err = imx290_write_table(priv, data_rate_table_720p[priv->data_rate]);
    if (err) {
        return err;
    }

    if (priv->data_rate == IMX290_HIGH_DATA_RATE) {
        if (s_data->numlanes == IMX290_TWO_LANE_MODE) {
            err = imx290_write_reg(s_data, FDGSEL_FRSEL, 0x01);
            if (err) {
                dev_err(dev, "%s: error setting FRSEL\n", __func__);
                return err;
            }
        }
        hmax = 1650 * csi_lane_koef;
    }
    else if (priv->data_rate == IMX290_MID_DATA_RATE) {
        if (s_data->numlanes == IMX290_TWO_LANE_MODE) {
            err = imx290_write_reg(s_data, FDGSEL_FRSEL, 0x02);
            if (err) {
                dev_err(dev, "%s: error setting FRSEL\n", __func__);
                return err;
            }
        }
        hmax = 3300 * csi_lane_koef;
    }
    else 
        hmax = 6600;
   
    err = imx290_write_buffered_reg(s_data, HMAX_LOW, 2, hmax);
    if (err)
        dev_err(dev, "%s: unable to write hmax\n", __func__);
...
static int imx290_set_data_rate_1080p(struct tegracam_device *tc_dev)
{
    struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
    struct device *dev = tc_dev->dev;
    struct camera_common_data *s_data = tc_dev->s_data;
    ...
    err = imx290_write_table(priv, data_rate_table[priv->data_rate]);
    if (err) {
        return err;
    }
...
/**
 * IMX290 allows for multiple data rates. Available data rates are described
 * in the chapter "Readout Drive mode" in the IMX290 datasheet
 */
static int imx290_set_data_rate(struct tegracam_device *tc_dev)
{
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
    ...
    err = imx290_verify_data_rate(tc_dev);
    if (err)
        goto fail;
    
    if (s_data->mode == IMX290_MODE_1280x720) {
        err = imx290_set_data_rate_720p(tc_dev);
    } else {
        err = imx290_set_data_rate_1080p(tc_dev);
    }
    if (err) 
        goto fail;
...
/**
 * XVS & XHS triggering pins
 *       XVS            XHS
 * 0x0 - fixed to Hi, fixed to Hi
 * 0x2 - output,      fixed to Hi
 * 0x8 - fixed to Hi, output
 * 0xA - output,      output
 */
static int imx290_configure_triggering_pins(struct tegracam_device *tc_dev)
{
	struct imx290 *priv = (struct imx290 *)tc_dev->priv;
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
    ...
    err = imx290_write_reg(s_data, XHSOUTSEL_XVSOUTSEL, xvs_xhs_drv);
    if (err) {
 ...
 */
static bool imx290_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx290 *priv = (struct imx290 *)tc_dev->priv;
    struct imx290 *current_priv;
	struct device *dev = tc_dev->dev;
    u8 streaming_mode = priv->current_streaming_mode;

    list_for_each_entry(current_priv, &imx290_sensor_list, entry) {
 ...
 */
static bool imx290_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx290 *priv = (struct imx290 *)tc_dev->priv;
    struct imx290 *current_priv; 
	struct device *dev = tc_dev->dev;
...
static int imx290_set_mode(struct tegracam_device *tc_dev)
{
    ...
    err = imx290_write_table(priv, mode_table[IMX290_INIT_SETTINGS]);
    if (err) {
        dev_err(dev, "%s: unable to initialize sensor settings\n", __func__);
        return err;
    }

    err = imx290_set_csi_lane_mode(tc_dev);
    if (err) {
        dev_err(dev, "%s: error setting CSI lane mode\n", __func__);
        return err;
    }

    err = imx290_set_pixel_format(tc_dev);
    if (err) {
    ...
    err = imx290_set_operation_mode(tc_dev, priv->current_operation_mode);
    if (err) {
        dev_err(dev, "%s: unable to set operation mode\n", __func__);
        return err;   
    }
    
    err = imx290_configure_triggering_pins(tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to configure XVS/XHS pins\n", __func__);
        return err;   
    }

    err = imx290_set_data_rate(tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to set data rate\n", __func__);
        return err;
    }
    ...
    if (!imx290_is_stream_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
        return -EPERM;
    }

    if (!imx290_is_pin_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal pin configuration detected\n", __func__);
        return -EPERM;
    }

    err = imx290_calculate_line_time(tc_dev);
    if (err) 
        return err;

    err = imx290_update_framerate_range(tc_dev);
    if (err) 
        return err;
...
static int imx290_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx290_write_table(priv,
		mode_table[IMX290_MODE_START_STREAM]);
	if (err)
		return err;
...
static int imx290_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx290_write_table(priv, mode_table[IMX290_MODE_STOP_STREAM]);
	if (err)
		return err;
	 ...
	 */
	usleep_range(priv->frame_length * priv->line_time / IMX290_K_FACTOR, 
                priv->frame_length * priv->line_time / IMX290_K_FACTOR + 1000);
...
static int imx290_board_setup(struct imx290 *priv)
{
    ...
    err = imx290_communication_verify(priv->tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to communicate with sensor\n",  __func__);
        goto error;
    }

    err = imx290_calculate_line_time(priv->tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to calculate line time\n", __func__);
        goto error;
    }

    priv->min_frame_length = IMX290_DEFAULT_HEIGHT 
                                + IMX290_MIN_FRAME_DELTA_1080p;
...
static int imx290_ctrls_init(struct tegracam_device *tc_dev)
{
	struct imx290 *priv = (struct imx290 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = priv->s_data;
    ...
    numctrls = ARRAY_SIZE(imx290_custom_ctrl_list);

    for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx290_custom_ctrl_list[i];
			...
			const struct i2c_device_id *id)
{
    ...
    priv->s_data->blklvl_max_range = IMX290_MAX_BLACK_LEVEL;
    ...
    err = imx290_ctrls_init(tc_dev);
	if (err) {
		dev_err(dev, "tegra camera custom ctrl init failed\n");
		return err;
	}

    err = imx290_update_ctrl(tc_dev, priv->black_level);
    if (err)
		return err;
    ...
    list_add_tail(&priv->entry, &imx290_sensor_list);
