...
static struct v4l2_ctrl_config imx585_custom_ctrl_list[] = {
    {
        ...
        .min = 0,
        .max = ARRAY_SIZE(imx585_data_rate_menu) - 1,
        .def = 0,
        .qmenu = imx585_data_rate_menu,
    },
    {
        .ops = &imx585_custom_ctrl_ops,
...
static int imx585_update_ctrl(struct tegracam_device *tc_dev, int ctrl_id, u64 current_val, u64 default_val, u64 min_val, u64 max_val)
{
 ...
 * Set streaming mode 
 */
static int imx585_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx585 *priv = (struct imx585 *)tc_dev->priv;
...
static int imx585_set_sync_feature(struct tegracam_device *tc_dev, u32 val)
{
	struct imx585 *priv = (struct imx585 *)tc_dev->priv;
...
static struct tegracam_ctrl_ops imx585_ctrl_ops = {
    ...
    .set_streaming_mode = imx585_set_streaming_mode,
    .set_operation_mode = imx585_set_operation_mode,
    .set_sync_feature = imx585_set_sync_feature,
...
static int imx585_power_on(struct camera_common_data *s_data)
{
    ...
    /* Power ON sequence according to IMX585 datasheet */
...
static int imx585_power_off(struct camera_common_data *s_data)
{
     ...
     * in the chapter "XVS/XHS IO setting" in the IMX585 Synchronizing 
     * Sensors App Note
     */         
    ...
    /* Power OFF sequence according to IMX585 datasheet */
	...
	if (s_data->numlanes == IMX585_TWO_LANE_MODE) {
        ...
        switch(priv->data_rate) {
			...
			case IMX585_594_MBPS:
				...
				if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10) {
					priv->data_rate = IMX585_1440_MBPS;
					goto modify_ctrl;
				}
				else {
					priv->data_rate = IMX585_1782_MBPS;
					goto modify_ctrl;
				}
				break;
			case IMX585_1782_MBPS:
		...
		switch(priv->data_rate) {
			...
			case IMX585_1782_MBPS:
				if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10) {
					priv->data_rate = IMX585_2376_MBPS;
					goto modify_ctrl;
				}
				break;
			case IMX585_1188_MBPS:
...
/**
 * IMX585 allows for multiple data rates. Available data rates are described
 * in the chapter "Readout Drive mode" in the IMX585 datasheet
 */
static int imx585_set_data_rate(struct tegracam_device *tc_dev)
{
	struct imx585 *priv = (struct imx585 *)tegracam_get_privdata(tc_dev);
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
    ...
    err = imx585_write_reg(s_data, DATARATE_SEL, priv->data_rate);
    if (err) 
        goto fail;
 ...
 */
static int imx585_set_sync_mode(struct tegracam_device *tc_dev)
{
	struct imx585 *priv = (struct imx585 *)tc_dev->priv;
    struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
 ...
 */
static bool imx585_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx585 *priv = (struct imx585 *)tc_dev->priv;
    struct imx585 *current_priv;
	struct device *dev = tc_dev->dev;
    u8 streaming_mode = priv->current_streaming_mode;

    list_for_each_entry(current_priv, &imx585_sensor_list, entry) {
 ...
 */
static bool imx585_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx585 *priv = (struct imx585 *)tc_dev->priv;
    struct imx585 *current_priv; 
	struct device *dev = tc_dev->dev;
 ...
 */
int imx585_check_unsupported_mode(struct camera_common_data *s_data,
...
int imx585_after_set_pixel_format(struct camera_common_data *s_data)
{
	struct device *dev = s_data->dev;
	...
	if(priv->current_pixel_format != s_data->colorfmt->code) {
		...
		switch (s_data->colorfmt->code) {
			case MEDIA_BUS_FMT_SRGGB10_1X10:
				err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL, (*ctrl->p_cur.p_s64 >> 2),
										IMX585_DEFAULT_BLACK_LEVEL_10BPP, 0, IMX585_MAX_BLACK_LEVEL_10BPP);
				priv->current_pixel_format = MEDIA_BUS_FMT_SRGGB10_1X10;
				break;
			case MEDIA_BUS_FMT_SRGGB12_1X12:
				err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL, (*ctrl->p_cur.p_s64 << 2),
										 IMX585_DEFAULT_BLACK_LEVEL_12BPP, 0, IMX585_MAX_BLACK_LEVEL_12BPP);
...
static int imx585_set_mode(struct tegracam_device *tc_dev)
{
    ...
    err = imx585_set_operation_mode(tc_dev, priv->current_operation_mode);
    if (err) {
        dev_err(dev, "%s: unable to operation mode\n", __func__);
        return err;   
    }

    err = imx585_set_sync_mode(tc_dev);
    if (err) {
    ...
    err = imx585_set_data_rate(tc_dev);
    if (err) {
        dev_err(dev, "%s: unable to set data rate\n", __func__);
        return err;
    }
    ...
    if (!imx585_is_stream_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
        return -EPERM;
    }

    if (!imx585_is_pin_configuration_valid(tc_dev)) {
...
static int imx585_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx585_write_table(priv,
		mode_table[IMX585_MODE_START_STREAM]);
	if (err)
		return err;
			...
			const struct i2c_device_id *id)
{
    ...
    err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL, IMX585_DEFAULT_BLACK_LEVEL_12BPP, IMX585_DEFAULT_BLACK_LEVEL_12BPP, 0, IMX585_MAX_BLACK_LEVEL_12BPP);
    if (err)
		return err;
