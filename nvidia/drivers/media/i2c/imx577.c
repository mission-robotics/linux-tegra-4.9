...
static int imx577_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx577_custom_ctrl_ops = {
	.s_ctrl = imx577_set_custom_ctrls,
};
...
static struct v4l2_ctrl_config imx577_custom_ctrl_list[] = {
    {
        .ops = &imx577_custom_ctrl_ops,
        .id = TEGRA_CAMERA_CID_TEST_PATTERN,
        .name = "Test Pattern",
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx577_test_pattern_menu) - 1,
        .def = 0,
        .qmenu = imx577_test_pattern_menu,
    },
};
...
static int imx577_update_ctrl(struct tegracam_device *tc_dev, u64 val)
{
...
static int imx577_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
    ...
    err = imx577_write_buffered_reg(s_data, MANUAL_DATA_PEDESTAL_VALUE_LOW, 2, val);
	if (err)
        goto fail;
 ...
 * in the IMX577 software reference manual
 */
...
static int imx577_set_gain(struct tegracam_device *tc_dev, s64 val)
{
    ...
    if (val > IMX577_MAX_ANALOG_GAIN_TIMES) {
        digital_part = (val * IMX577_DIGITAL_GAIN_COEF + IMX577_MAX_ANALOG_GAIN_TIMES/2)
...
static int imx577_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	else if (integration_time_line > (priv->frame_length - IMX577_MIN_INTEGRATION_LINES_OFFSET))
 ...
 * Set streaming mode 
 */
static int imx577_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx577 *priv = (struct imx577 *)tc_dev->priv;
...
static struct tegracam_ctrl_ops imx577_ctrl_ops = {
    ...
    .set_streaming_mode = imx577_set_streaming_mode,
...
static int imx577_power_on(struct camera_common_data *s_data)
{
    ...
    /* Power ON sequence according to IMX577 datasheet */

	if (pw->dvdd) {
		err = regulator_enable(pw->dvdd);
		if (err)
			goto imx577_dvdd_fail;
	}

	if (pw->iovdd) {
		err = regulator_enable(pw->iovdd);
		if (err)
			goto imx577_iovdd_fail;
	}

	if (pw->avdd) {
		err = regulator_enable(pw->avdd);
		if (err)
			goto imx577_avdd_fail;
	}
    ...
    /* Must be set first after power on according to EXCK_FREQ entry in IMX577 register map */
...
static int imx577_power_off(struct camera_common_data *s_data)
{
    ...
    /* Power OFF sequence according to IMX577 datasheet */
...
static int imx577_communication_verify(struct tegracam_device *tc_dev)
{
	...
	err = imx577_read_buffered_reg(s_data, DT_PEDESTAL_LOW, 2, &black_level);
    if (err) {
...
static int imx577_configure_triggering_pins(struct tegracam_device *tc_dev)
{
    ...
    case MASTER_MODE:
        ...
        if (priv->current_streaming_mode == SYNC_STREAM) {
            ...
            /* output disable */
             err |= imx577_write_reg(s_data, EXTOUT_EN, 0); 
        }
 ...
 */
static bool imx577_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx577 *priv = (struct imx577 *)tc_dev->priv;
    struct imx577 *current_priv;
	struct device *dev = tc_dev->dev;
    u8 streaming_mode = priv->current_streaming_mode;

    list_for_each_entry(current_priv, &imx577_sensor_list, entry) {
 ...
 */
static bool imx577_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx577 *priv = (struct imx577 *)tc_dev->priv;
    struct imx577 *current_priv; 
	struct device *dev = tc_dev->dev;
 ...
 */
int imx577_check_unsupported_mode(struct camera_common_data *s_data,
                                     struct v4l2_mbus_framefmt *mf)
{
     ...
     * Table 5 "Typical image output of main capture mode" in IMX577 datasheet
     */
...
static int imx577_set_mode(struct tegracam_device *tc_dev)
{
    ...
    err = imx577_set_operation_mode(tc_dev, priv->current_operation_mode);
    if (err) {
    ...
    if (!imx577_is_stream_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
        return -EPERM;
    }

    if (!imx577_is_pin_configuration_valid(tc_dev)) {
...
static int imx577_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx577_write_table(priv,
		mode_table[IMX577_MODE_START_STREAM]);
	if (err)
		return err;
...
static int imx577_ctrls_init(struct tegracam_device *tc_dev)
{
	struct imx577 *priv = (struct imx577 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = priv->s_data;
    ...
    numctrls = ARRAY_SIZE(imx577_custom_ctrl_list);

    for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx577_custom_ctrl_list[i];
			...
			const struct i2c_device_id *id)
{
    ...
    priv->s_data->blklvl_max_range = IMX577_MAX_BLACK_LEVEL;
    ...
    err = imx577_ctrls_init(tc_dev);
	if (err) {
		dev_err(dev, "tegra camera custom ctrl init failed\n");
		return err;
	}

    err = imx577_update_ctrl(tc_dev, priv->black_level);
    if (err)
		return err;
...
static int
imx577_remove(struct i2c_client *client)
{
