...
#define IMX334_MAX_BLACK_LEVEL_12BIT 0xFFC
#define IMX334_MAX_BLACK_LEVEL_10BIT 0x3FF
...
static int imx334_update_ctrl(struct tegracam_device *tc_dev, u64 val)
{
...
static int imx334_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	...
	switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			...
			break;
		default:
			if (val > IMX334_MAX_BLACK_LEVEL_10BIT)
				val = IMX334_MAX_BLACK_LEVEL_10BIT;
			err = imx334_update_ctrl(tc_dev, val);
			black_level_reg = val;
	}

    err = imx334_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, black_level_reg);
	if (err){
...
static int imx334_communication_verify(struct tegracam_device *tc_dev)
{
	...
	err = imx334_read_buffered_reg(s_data, BLKLEVEL_LOW, 2, &black_level);
    if (err) {
			...
			const struct i2c_device_id *id)
{
    ...
    priv->s_data->blklvl_max_range = IMX334_MAX_BLACK_LEVEL_12BIT;
    ...
    err = imx334_update_ctrl(tc_dev, priv->black_level);
    if (err)
		return err;
