			     ...
			     struct register_access *ra_p)
{
	...
	ret = basler_read_register_chunk(client, ra_p->data,
					 sensor->ra_tmp.data_size,
					 sensor->ra_tmp.address);
	if (unlikely(ret < 0))
		ra_p->data_size = 0;
	else
		ra_p->data_size = ret;
				   ...
				   struct basler_device_information *bdi)
{
	...
	if (unlikely(ret != sizeof(deviceCapabilitiesBe))) {
	...
	if (unlikely(ret != sizeof(gencpVersionBe))) {
		pr_err("basler_read_register() not read full buffer = %d bytes\n",
		       ret);
		return -EIO;
	}
	...
	if (unlikely(ret != GENCP_STRING_BUFFER_SIZE)) {
		pr_err("basler_read_register() not read full buffer = %d bytes\n",
		       ret);
		return -EIO;
	}
...
static int basler_camera_enum_mbus_code(struct v4l2_subdev *sd,
					struct v4l2_subdev_pad_config *cfg,
					struct v4l2_subdev_mbus_code_enum *code)
{
	static uint16_t supported_datatypes[] = {
		MEDIA_BUS_FMT_RGB888_1X24, // RGB888
		MEDIA_BUS_FMT_UYVY8_1X16, // YUV422 8-bit
		MEDIA_BUS_FMT_YUYV8_2X8, // YUV422 8-bit
		MEDIA_BUS_FMT_SRGGB8_1X8, // RAW8
		MEDIA_BUS_FMT_SRGGB10_1X10, // RAW10
		MEDIA_BUS_FMT_SRGGB12_1X12 // RAW12
	};

	if (code->index >= ARRAY_SIZE(supported_datatypes))
		return -EINVAL;

	code->code = supported_datatypes[code->index];

	v4l2_info(sd, "%s: index=%u -> code=%04X\n", __func__, code->index,
		  code->code);
...
/*
 * basler_camera_enum_framesizes() - returns the supported frame sizes by index.
 * @sd  the v4l2 subdevice
 * @cfg the pad configuration
 * @fse the frame size returned.
 *
 * Return 0 on success and an error otherwise.
 */
static int
basler_camera_enum_framesizes(struct v4l2_subdev *sd,
			      struct v4l2_subdev_pad_config *cfg,
			      struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->code != V4L2_PIX_FMT_UYVY &&
		fse->code != V4L2_PIX_FMT_SRGGB8 &&
		fse->code != V4L2_PIX_FMT_ABGR32 &&
		fse->code != V4L2_PIX_FMT_GREY)
		return -EINVAL;
#endif
	if (fse->index >= 1)
		return -EINVAL;
	...
	v4l2_info(sd, "%s (...) => %d/%d .. %d/%d\n", __func__, fse->min_width,
		  fse->min_height, fse->max_width, fse->max_height);
...
static int basler_camera_get_detected_timings(struct v4l2_subdev *sd,
					      struct v4l2_dv_timings *timings)
{
	...
	bt->interlaced = V4L2_DV_PROGRESSIVE; // V4L2_DV_INTERLACED
	bt->width = 1920;
	bt->height = 1080;
	bt->vsync = 1;
	bt->hsync = 1;
	bt->pixelclock = bt->width * bt->height * 30;

	return 0;
}

static int basler_camera_s_dv_timings(struct v4l2_subdev *sd,
				      struct v4l2_dv_timings *timings)
{
	v4l2_err(sd, "%s():\n", __func__);
	return 0;
...
static int basler_camera_g_dv_timings(struct v4l2_subdev *sd,
				      struct v4l2_dv_timings *timings)
{
	v4l2_err(sd, "%s():\n", __func__);
	return basler_camera_get_detected_timings(sd, timings);
}

static int basler_camera_query_dv_timings(struct v4l2_subdev *sd,
					  struct v4l2_dv_timings *timings)
{
	int ret;
	...
	ret = basler_camera_get_detected_timings(sd, timings);
	if (ret < 0)
		return ret;
...
static int basler_camera_g_mbus_config(struct v4l2_subdev *sd,
...
static const struct v4l2_subdev_video_ops basler_camera_video_ops = {
	...
	.s_dv_timings = basler_camera_s_dv_timings,
	.g_dv_timings = basler_camera_g_dv_timings,
	.query_dv_timings = basler_camera_query_dv_timings,
	.g_mbus_config = basler_camera_g_mbus_config,
};

static const struct v4l2_subdev_pad_ops basler_camera_pad_ops = {
	.set_fmt = basler_camera_set_fmt,
	.enum_mbus_code = basler_camera_enum_mbus_code,
	.enum_frame_size = basler_camera_enum_framesizes,
};
				       ...
				       struct device *dev)
{
	...
	if (unlikely(ctrl == NULL)) {
		dev_err(dev, "Registering ctrl %s failed: %d\n", cfg->name,
			hdl->error);
		ret = hdl->error;
	}
...
static int basler_camera_init_controls(struct basler_camera_dev *sensor)
{
	...
	if (unlikely(basler_camera_alloc_control(hdl, &ctrl_access_register,
						 &sensor->i2c_client->dev) ||
		     basler_camera_alloc_control(
			     hdl, &ctrl_basler_device_information,
			     &sensor->i2c_client->dev) ||
		     basler_camera_alloc_control(hdl,
						 &ctrl_basler_interface_version,
						 &sensor->i2c_client->dev) ||
		     basler_camera_alloc_control(hdl,
						 &ctrl_basler_csi_information,
						 &sensor->i2c_client->dev))) {
			       ...
			       const struct i2c_device_id *id)
{
	...
	if (unlikely(ret)) {
		dev_dbg(dev, "basler_camera_init_controls() failed: %d\n", ret);
		goto entity_cleanup;
	}

	ret = v4l2_async_register_subdev(&sensor->sd);
	if (unlikely(ret)) {
		dev_dbg(dev, "v4l2_async_register_subdev() failed: %d\n", ret);
		goto handler_cleanup;
	}

	dev_dbg(dev, " %s driver probed\n", SENSOR_NAME);
	dev_info(dev, "Basler Camera Driver v" DRIVER_VERSION
		      " loaded successfully\n");
	return 0;
