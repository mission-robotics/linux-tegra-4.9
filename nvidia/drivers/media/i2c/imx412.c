...
static int imx412_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx412_custom_ctrl_ops = {
	.s_ctrl = imx412_set_custom_ctrls,
};
...
static struct v4l2_ctrl_config imx412_custom_ctrl_list[] = {
    {
        .ops = &imx412_custom_ctrl_ops,
        .id = TEGRA_CAMERA_CID_TEST_PATTERN,
        .name = "Test Pattern",
        .type = V4L2_CTRL_TYPE_MENU,
        .min = 0,
        .max = ARRAY_SIZE(imx412_test_pattern_menu) - 1,
        .def = 0,
        .qmenu = imx412_test_pattern_menu,
    },
};
...
static int imx412_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
                                ...
                                u16 addr_low, u8 number_of_registers, u64 val)
{
    ...
    for (i = 0; i < number_of_registers; i++) {
        err = imx412_write_reg(s_data, addr_low - i, (u8)(val >> (i * 8)));
        if (err) {
...
static int imx412_write_table(struct imx412 *priv,
				const imx412_reg table[])
{
...
static int imx412_update_ctrl(struct tegracam_device *tc_dev, u64 val)
{
...
static int imx412_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
    ...
    err = imx412_write_buffered_reg(s_data, MANUAL_DATA_PEDESTAL_VALUE_LOW, 2, val);
	if (err)
        goto fail;
 ...
 * in the IMX412 software reference manual
 */
static int imx412_get_digital_gain(struct tegracam_device *tc_dev, s64 val)
{
    ...
    decimal_part = ( (val % IMX412_DIGITAL_GAIN_COEF) * 10 )  
                                                / IMX412_DIGITAL_GAIN_STEP;
...
static int imx412_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	/* translate value */
	analog_gain = IMX412_ANALOG_GAIN_OFFSET - (IMX412_ANALOG_GAIN_OFFSET 
                                * mode->control_properties.gain_factor) / val;
    analog_gain =  (analog_gain > IMX412_MAX_ANALOG_GAIN)  ? 
                            IMX412_MAX_ANALOG_GAIN : analog_gain;
    
    /* digital gain */    
    if (val > IMX412_MAX_ANALOG_GAIN_TIMES) {
        digital_part = (val * IMX412_DIGITAL_GAIN_COEF + IMX412_MAX_ANALOG_GAIN_TIMES/2)
    ...
    if (priv->broadcast_ctrl == BROADCAST) {
        ...
        for (i = 0; i < 2; i++)
            err |= imx412_write_reg_broadcast(s_data, ANA_GAIN_GLOBAL_LOW - i, 
               (u8)(analog_gain >> (i * 8)));

        for (i = 0; i < 2; i++)
            err |= imx412_write_reg_broadcast(s_data, DIG_GAIN_GR_LOW - i, 
               (u8)(digital_gain >> (i * 8)));
...
static int imx412_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	integration_time_line = ((val - priv->integration_offset)
                                     * IMX412_K_FACTOR) / priv->line_time ;
	...
	else if (integration_time_line > (priv->frame_length - IMX412_MIN_INTEGRATION_LINES_OFFSET)) 
		integration_time_line = priv->frame_length - IMX412_MIN_INTEGRATION_LINES_OFFSET;

    if (priv->broadcast_ctrl == BROADCAST)       
        err = imx412_broadcast_buffered_reg(s_data, COARSE_INTEG_TIME_LOW, 2,
        ...
        for (i = 0; i < 2; i++)
            err |= imx412_write_reg(s_data, COARSE_INTEG_TIME_LOW - i, 
...
static int imx412_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
    ...
    frame_length = (((u64)mode->control_properties.framerate_factor * 
                              IMX412_G_FACTOR) / (val * priv->line_time));
    ...
    exposure_min_range = IMX412_MIN_INTEGRATION_LINES 
                                            * priv->line_time / IMX412_K_FACTOR;
    exposure_min_range += priv->integration_offset;   

    exposure_max_range = (priv->frame_length - IMX412_MIN_INTEGRATION_LINES_OFFSET) 
                                            * priv->line_time / IMX412_K_FACTOR;
    ...
    for (i = 0; i < 2; i++)
        err |= imx412_write_reg(s_data, FRM_LENGTH_LINES_LOW - i, 
...
static int imx412_update_framerate_range(struct tegracam_device *tc_dev)
{
    ...
    case IMX412_MODE_SCALE_HV3:
        priv->min_frame_length = IMX412_DEFAULT_HEIGHT 
                                + IMX412_MIN_FRAME_LENGTH_DELTA;
        break;
	default:
 ...
 * Set streaming mode 
 */
static int imx412_set_streaming_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx412 *priv = (struct imx412 *)tc_dev->priv;
...
static struct tegracam_ctrl_ops imx412_ctrl_ops = {
    ...
    .set_streaming_mode = imx412_set_streaming_mode,
...
static int imx412_power_on(struct camera_common_data *s_data)
{
    ...
    /* Power ON sequence according to IMX412 datasheet */
    ...
    /* Must be set first after power on according to EXCK_FREQ entry in IMX412 register map */
...
static int imx412_power_off(struct camera_common_data *s_data)
{
    ...
    /* Power OFF sequence according to IMX412 datasheet */
...
static int imx412_communication_verify(struct tegracam_device *tc_dev)
{
	...
	err = imx412_read_buffered_reg(s_data, DT_PEDESTAL_LOW, 2, &black_level);
    if (err) {
...
static struct camera_common_pdata *imx412_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
static int imx412_calculate_line_time(struct tegracam_device *tc_dev)
{
    ...
    //line_length
    err = imx412_read_buffered_reg(s_data, LINE_LENGTH_PCK_LOW, 2, &line_length);
    if (err)
        goto fail; 
    ...
    ivtpxck = ivtck / (ivt_syck_div * IMX412_IVT_PXCK_DIV_VAL) / IMX412_M_FACTOR;
    priv->line_time = ((line_length / IMX412_IMAGE_PIPE_LINES) 
                                            * IMX412_K_FACTOR) / ivtpxck;   

    priv->integration_offset = ( IMX412_FINE_STORAGE_TIME * priv->line_time ) / 
...
static int imx412_adjust_hmax_register(struct tegracam_device *tc_dev)
{
    ...
    if(s_data->numlanes == 2){
        imx412_read_buffered_reg(s_data, LINE_LENGTH_PCK_LOW, 2, &line_length);
            
        line_length *=2;

        err = imx412_write_buffered_reg(s_data, LINE_LENGTH_PCK_LOW, 
                                                            2, line_length);
        if (err) {
...
static void imx412_enable_second_slave_address(struct imx412 *ack_private)
{
    ...
    err = imx412_write_reg(ack_private->s_data, 
...
static void imx412_disable_second_slave_address(void)
{
    ...
    list_for_each_entry(current_private, &imx412_sensor_list, entry) {
        ...
        err = imx412_write_reg(current_private->s_data, SLAVE_ADD_EN_2ND, 0);
        if (err)
            dev_warn(&current_private->i2c_client->dev, 
                "%s: Fail to write Second I2C register\n", __func__);
...
static int imx412_configure_triggering_pins(struct tegracam_device *tc_dev)
{
    ...
    case MASTER_MODE:
        ...
        if (priv->current_streaming_mode == SYNC_STREAM) {
            ...
            /* output disable */
             err |= imx412_write_reg(s_data, EXTOUT_EN, 0); 
        }
 ...
 */
static bool imx412_is_stream_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx412 *priv = (struct imx412 *)tc_dev->priv;
    struct imx412 *current_priv;
	struct device *dev = tc_dev->dev;
    u8 streaming_mode = priv->current_streaming_mode;

    list_for_each_entry(current_priv, &imx412_sensor_list, entry) {
 ...
 */
static bool imx412_is_pin_configuration_valid(struct tegracam_device *tc_dev)
{
	struct imx412 *priv = (struct imx412 *)tc_dev->priv;
    struct imx412 *current_priv; 
	struct device *dev = tc_dev->dev;
 ...
 */
int imx412_check_unsupported_mode(struct camera_common_data *s_data,
                                     struct v4l2_mbus_framefmt *mf)
{
     ...
     * Table 5 "Typical image output of main capture mode" in IMX412 datasheet
     */
...
static int imx412_set_mode(struct tegracam_device *tc_dev)
{
    ...
    err = imx412_set_operation_mode(tc_dev, priv->current_operation_mode);
    if (err) {
    ...
    if (!imx412_is_stream_configuration_valid(tc_dev)) {
        dev_err(dev, "%s: illegal stream configuration detected\n", __func__);
        return -EPERM;
    }

    if (!imx412_is_pin_configuration_valid(tc_dev)) {
...
static int imx412_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx412_write_table(priv,
		mode_table[IMX412_MODE_START_STREAM]);
	if (err)
		return err;
...
static int imx412_ctrls_init(struct tegracam_device *tc_dev)
{
	struct imx412 *priv = (struct imx412 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = priv->s_data;
    ...
    numctrls = ARRAY_SIZE(imx412_custom_ctrl_list);

    for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx412_custom_ctrl_list[i];
			...
			const struct i2c_device_id *id)
{
    ...
    priv->s_data->blklvl_max_range = IMX412_MAX_BLACK_LEVEL;
    ...
    err = imx412_ctrls_init(tc_dev);
	if (err) {
		dev_err(dev, "tegra camera custom ctrl init failed\n");
		return err;
	}

    err = imx412_update_ctrl(tc_dev, priv->black_level);
    if (err)
		return err;
...
static int
imx412_remove(struct i2c_client *client)
{
